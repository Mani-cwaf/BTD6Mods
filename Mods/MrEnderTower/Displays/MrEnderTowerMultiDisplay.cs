...
namespace MrEnderTower.Displays
{
    ...
    /// </summary>
    public class MrEnderTowerMultiDisplay : ModTowerDisplay<MrEnderTower>
    {
        private readonly int[] t;  // The tiers used for a particular instance of MrEnderTowerBaseDisplay

        /// <summary>
        /// All classes that derive from ModContent MUST have a zero argument constructor to work
        /// </summary>
        public MrEnderTowerMultiDisplay()
        {

        }

        public MrEnderTowerMultiDisplay(int[] tiers)
        {
            t = tiers;
        }
            ...
            for (var i = 0; i <= 2; i++)
            {
                for (var j = 0; j <= 2; j++)
                {
                    for (var k = 0; k <= 2; k++)
                    {
                        ...
                        if (tiers.IsValid())
                        {
                            yield return new MrEnderTowerMultiDisplay(tiers);
                        }
                    }
                }
            }
        ...
        public override string Name => nameof(MrEnderTowerMultiDisplay) + "-" + t.Printed();

        // Copy the corresponding Boomerang Monkey display
        public override string BaseDisplay => GetDisplay(TowerType.EngineerMonkey, t[0], t[1], t[2]);
        ...
        public override void ModifyDisplayNode(UnityDisplayNode node)
        {
            ...
            SetMeshTexture(node, nameof(MrEnderTowerBaseDisplay));
